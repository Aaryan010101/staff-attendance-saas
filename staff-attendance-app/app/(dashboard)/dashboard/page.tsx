...
import type { Staff, Attendance, SalaryRecord } from '@/types/database';
...
export default async function DashboardPage() {
  ...
  const staffIds = activeStaff.map((s: Staff) => s.id);

  // Today's attendance
  const { data: todayAtt } = staffIds.length > 0
    ? await supabaseAdmin
        .from('attendance')
        .select('*')
  ...
  const presentToday = (todayAtt ?? []).filter((a: Attendance) => a.status === 'present').length;
  const absentToday = (todayAtt ?? []).filter((a: Attendance) => a.status === 'absent').length;
  const halfToday = (todayAtt ?? []).filter((a: Attendance) => a.status === 'half_day').length;
  const notMarked = activeStaff.length - (todayAtt ?? []).length;

  // Monthly payroll estimate
  const { data: monthlyAtt } = staffIds.length > 0
    ? await supabaseAdmin
        .from('attendance')
  ...
  for (const s of activeStaff) {
    if (s.salary_type === 'monthly') {
      ...
      const presenceDays = (monthlyAtt ?? [])
        .filter((a: { staff_id: string; status: string }) => a.staff_id === s.id)
        .reduce((acc: number, a: { status: string }) => acc + (a.status === 'present' ? 1 : a.status === 'half_day' ? 0.5 : 0), 0);
      payrollEstimate += (s.daily_wage ?? 0) * presenceDays;
    }
  }

  // Pending salary payments
  const { data: salaryRecords } = staffIds.length > 0
    ? await supabaseAdmin
        .from('salary_records')
        .select('*, staff!inner(name, role)')
        .in('staff_id', staffIds)
        ...
        <section>
          ...
          {(salaryRecords ?? []).length === 0 ? (
            ...
            <div className="space-y-2">
              {(salaryRecords as (SalaryRecord & { staff: { name: string; role: string } })[]).map((rec) => (
